...
class ClientEventTests: XCTestCase {
    var connection: MockEventflitConnection!
    ...
    override func setUp() {
        ...
        connection = MockEventflitConnection(options: EventflitClientOptions(authMethod: .inline(secret: "superSecretSecret")))
        socket.delegate = connection
        connection.socket = socket
    }

    func testTriggeringClientEventsDoesNotWriteToTheSocketForPublicChannels() {
        let chan = EventflitChannel(name: "test-channel", connection: connection)
    ...
    func testTriggeringClientEventsWritesToTheSocketForAuthenticatedChannels() {
        let chan = EventflitChannel(name: "private-channel", connection: connection)
        chan.subscribed = true
        chan.trigger(eventName: "client-test-event", data: ["data": "testing client events"])
