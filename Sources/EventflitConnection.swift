...
@objc open class EventflitConnection: NSObject {
    ...
    open lazy var reachability: Reachability? = {
        ...
        reachability?.whenReachable = { [weak self] reachability in
            guard self != nil else {
                print("Your Eventflit instance has probably become deallocated. See https://github.com/eventflit/eventflit-websocket-swift/issues/109 for more information")
                return
            }

            self!.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Network reachable")
            ...
            case .connected:
                ...
                self!.delegate?.debugLog?(
                    message: "[EVENTFLIT DEBUG] Connection state is \(self!.connectionState.stringValue()) but received network reachability change so going to call attemptReconnect"
                )
        ...
        reachability?.whenUnreachable = { [weak self] reachability in
            ...
            self!.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Network unreachable")
            self!.resetConnectionAndAttemptReconnect()
        }
        return reachability
    }()
    ...
    /**
        Initializes a new EventflitConnection with an app key, websocket, URL, options and URLSession

        - parameter key:        The Eventflit app key
        - parameter socket:     The websocket object
        - parameter url:        The URL the connection is made to
        - parameter options:    A EventflitClientOptions instance containing all of the user-speficied
                                client options
        - parameter URLSession: An NSURLSession instance for the connection to use for making
                                authentication requests

        - returns: A new EventflitConnection instance
    */
    public init(
        key: String,
        socket: WebSocket,
    ...
    /**
        Initializes a new EventflitChannel with a given name
    ...
    */
    ...
    /**
        Unsubscribes from all EventflitChannels
    */
    ...
    /**
        ...
        - parameter channelName: The name of the channel
    */
    open func sendEvent(event: String, data: Any, channel: EventflitChannel? = nil) {
        if event.components(separatedBy: "-")[0] == "client" {
            ...
            self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] sendEvent \(dataString)")
    ...
    /**
        ...
        - parameter channelName: The name of the channel
    */
    fileprivate func sendClientEvent(event: String, data: Any, channel: EventflitChannel?) {
        if let channel = channel {
            if channel.type == .presence || channel.type == .private {
                ...
                self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] sendClientEvent \(dataString)")
                self.socket.write(string: dataString)
            } else {
    ...
    */
    @objc fileprivate func sendPing() {
        socket.write(ping: Data()) {
            self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Ping sent")
    ...
    */
    ...
    /**
        ...
        - parameter json: The EventflitEventJSON containing successful subscription data
    */
    fileprivate func handleSubscriptionSucceededEvent(json: EventflitEventJSON) {
        if let channelName = json["channel"] as? String, let chan = self.channels.find(name: channelName) {
            ...
            guard let eventData = json["data"] as? String else {
                self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Subscription succeeded event received without data key in payload")
                return
            }

            if EventflitChannelType.isPresenceChannel(name: channelName) {
                if let presChan = self.channels.find(name: channelName) as? EventflitPresenceChannel {
                    if let dataJSON = getEventflitEventJSON(from: eventData) {
            ...
            callGlobalCallbacks(forEvent: "eventflit:subscription_succeeded", jsonObject: json)
            chan.handleEvent(name: "eventflit:subscription_succeeded", data: eventData)
            ...
            while chan.unsentEvents.count > 0 {
                if let eventflitEvent = chan.unsentEvents.popLast() {
                    chan.trigger(eventName: eventflitEvent.name, data: eventflitEvent.data)
                }
            }
        }
    }
    ...
    /**
        ...
        - parameter json: The EventflitEventJSON containing connection established data
    */
    fileprivate func handleConnectionEstablishedEvent(json: EventflitEventJSON) {
        if let data = json["data"] as? String {
            if let connectionData = getEventflitEventJSON(from: data),
               let socketId = connectionData["socket_id"] as? String
            {
                self.socketId = socketId
                self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Socket established with socket ID: \(socketId)")
    ...
    */
    ...
    /**
        ...
        - parameter json: The EventflitEventJSON containing the member data
    */
    fileprivate func handleMemberAddedEvent(json: EventflitEventJSON) {
        if let data = json["data"] as? String {
            if let channelName = json["channel"] as? String, let chan = self.channels.find(name: channelName) as? EventflitPresenceChannel {
                if let memberJSON = getEventflitEventJSON(from: data) {
    ...
    /**
        ...
        - parameter json: The EventflitEventJSON containing the member data
    */
    fileprivate func handleMemberRemovedEvent(json: EventflitEventJSON) {
        if let data = json["data"] as? String {
            if let channelName = json["channel"] as? String, let chan = self.channels.find(name: channelName) as? EventflitPresenceChannel {
                if let memberJSON = getEventflitEventJSON(from: data) {
    ...
    */
    fileprivate func handleAuthorizationError(forChannel channelName: String, response: URLResponse?, data: String?, error: NSError?) {
        let eventName = "eventflit:subscription_error"
        let json = [
            "event": eventName,
            "channel": channelName,
            "data": data ?? ""
    ...
    /**
        Parse a string to extract Eventflit event information from it

        - parameter string: The string received over the websocket connection containing
                            Eventflit event information

        - returns: A dictionary of Eventflit-relevant event data
    */
    open func getEventflitEventJSON(from string: String) -> [String : AnyObject]? {
    ...
    /**
        Parse a string to extract Eventflit event data from it

        - parameter string: The data string received as part of a Eventflit message

        - returns: The object sent as the payload part of the Eventflit message
    */
    ...
    */
    open func handleEvent(eventName: String, jsonObject: [String : AnyObject]) {
        ...
        switch eventName {
        case "eventflit_internal:subscription_succeeded":
            handleSubscriptionSucceededEvent(json: jsonObject)
        case "eventflit:connection_established":
            handleConnectionEstablishedEvent(json: jsonObject)
        case "eventflit_internal:member_added":
            handleMemberAddedEvent(json: jsonObject)
        case "eventflit_internal:member_removed":
    ...
    */
    ...
    /**
        ...
        - parameter channel: The EventflitChannel to authenticate
        - parameter auth:    A EventflitAuth value if subscription is being made to an
                             authenticated channel without using the default auth methods
        ...
        - returns: A Bool indicating whether or not the authentication request was made
                   successfully
    */
    fileprivate func authorize(_ channel: EventflitChannel, auth: EventflitAuth? = nil) -> Bool {
        if channel.type != .presence && channel.type != .private {
            ...
            if channel.type == .private {
                ...
                self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Attempting to subscribe to presence channel but no channelData value provided")
                return false
            }
            ...
            case .noMethod:
                ...
                let error = NSError(domain: "com.eventflit.EventflitSwift", code: 0, userInfo: [NSLocalizedFailureReasonErrorKey: errorMessage])
    ...
    */
    ...
    /**
        ...
        - parameter channel:  The EventflitChannel to subscribe to
    */
    fileprivate func subscribeToNormalChannel(_ channel: EventflitChannel) {
        self.sendEvent(
            event: "eventflit:subscribe",
            data: [
    ...
    /**
     Creates an authentication request for the given authEndpoint
        ...
        - parameter channel:  The EventflitChannel to authenticate subsciption for
    ...
    */
    ...
    /**
        ...
        - parameter channel: The EventflitChannel to authenticate subsciption for
    */
    fileprivate func sendAuthorisationRequest(request: URLRequest, channel: EventflitChannel) {
    ...
    /**
        ...
        - parameter channel: The EventflitChannel to authorize subsciption for
    */
    fileprivate func handleAuthResponse(
        json: [String: AnyObject],
        channel: EventflitChannel
    ) {
    ...
    /**
        ...
        - parameter channel:     The EventflitChannel to authorize the subsciption for
    */
    fileprivate func handleAuthInfo(authString: String, channelData: String?, channel: EventflitChannel) {
    ...
    /**
        Handle presence channel auth response and send subscribe message to Eventflit API
    ...
    */
    fileprivate func handlePresenceChannelAuth(
        ...
        (channel as? EventflitPresenceChannel)?.setMyUserId(channelData: channelData)
    ...
    /**
        Handle private channel auth response and send subscribe message to Eventflit API

        - parameter auth:    The auth string
        - parameter channel: The EventflitChannel to authenticate subsciption for
    */
    fileprivate func handlePrivateChannelAuth(authValue auth: String, channel: EventflitChannel) {
        self.sendEvent(
            event: "eventflit:subscribe",
            data: [
                "channel": channel.name,
...
@objc public class EventflitAuth: NSObject {
