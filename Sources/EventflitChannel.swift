...
public enum EventflitChannelType {
    case `private`
    case presence
    case normal
    ...
    public static func type(forName name: String) -> EventflitChannelType {
    ...
    public static func isPresenceChannel(name: String) -> Bool {
        return EventflitChannelType(name: name) == .presence
    }
}

@objcMembers
open class EventflitChannel: NSObject {
    open var eventHandlers: [String: [EventHandler]] = [:]
    open var subscribed = false
    open let name: String
    open weak var connection: EventflitConnection?
    open var unsentEvents = [EventflitEvent]()
    open let type: EventflitChannelType
    public var auth: EventflitAuth?

    /**
        Initializes a new EventflitChannel with a given name and conenction
    ...
    */
    public init(name: String, connection: EventflitConnection, auth: EventflitAuth? = nil) {
        self.name = name
        self.connection = connection
        self.auth = auth
        self.type = EventflitChannelType(name: name)
    }

    /**
        Binds a callback to a given event name, scoped to the EventflitChannel the function is
        called on
    ...
    */
    open func trigger(eventName: String, data: Any) {
        if subscribed {
            ...
            unsentEvents.insert(EventflitEvent(name: eventName, data: data), at: 0)
        }
    }
}
