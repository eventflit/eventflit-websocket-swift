...
extension EventflitConnection: WebSocketDelegate {
    ...
    */
    public func websocketDidReceiveMessage(socket ws: WebSocketClient, text: String) {
        self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] websocketDidReceiveMessage \(text)")
        if let eventflitPayloadObject = getEventflitEventJSON(from: text), let eventName = eventflitPayloadObject["event"] as? String {
            self.handleEvent(eventName: eventName, jsonObject: eventflitPayloadObject)
        } else {
            self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Unable to handle incoming Websocket message \(text)")
        }
    }
    ...
    */
    public func websocketDidDisconnect(socket ws: WebSocketClient, error: Error?) {
        ...
        guard !intentionalDisconnect else {
            self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Deliberate disconnection - skipping reconnect attempts")
            return
        }
        ...
        if let error = error {
            self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Websocket is disconnected. Error (code: \((error as NSError).code)): \(error.localizedDescription)")
        } else {
            self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Websocket is disconnected but no error received")
        }
        ...
        guard reconnectAttemptsMax == nil || reconnectAttempts < reconnectAttemptsMax! else {
            self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Max reconnect attempts reached")
            return
        }

        if let reachability = self.reachability, reachability.connection == .none {
            self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Network unreachable so reconnect likely to fail")
        }
    ...
    */
    internal func attemptReconnect() {
        ...
        if reconnectAttemptsMax != nil {
            self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Waiting \(timeInterval) seconds before attempting to reconnect (attempt \(reconnectAttempts + 1) of \(reconnectAttemptsMax!))")
        } else {
            self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Waiting \(timeInterval) seconds before attempting to reconnect (attempt \(reconnectAttempts + 1))")
        }
...
extension EventflitConnection: WebSocketPongDelegate {

    public func websocketDidReceivePong(socket: WebSocketClient, data: Data?) {
        self.delegate?.debugLog?(message: "[EVENTFLIT DEBUG] Websocket received pong")
